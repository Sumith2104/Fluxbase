  ...
  "inline-flex items-center transition-colors justify-center cursor-pointer gap-2 whitespace-nowrap rounded-xl text-sm font-medium transition-[color,box-shadow] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      ...
      size: {
        ...
        sm: "h-8 text-xs gap-1.5 px-4 has-[>svg]:px-4 rounded-xl",
        lg: "h-10 rounded-xl px-6 has-[>svg]:px-4",
        xl: "h-12 rounded-xl px-8 has-[>svg]:px-6",
        xxl: "h-14 rounded-2xl px-10 has-[>svg]:px-8",
  ...
  return (
    <>
      ...
      >
        ...
        <div className="pointer-events-none z-10 inline-flex items-center justify-center gap-1.5 w-full h-full">
          {children}
        </div>
        <GlassFilter />
      </Comp>
...
const metalButtonVariants = (
  ...
  return {
    ...
    button: cn(
      "relative z-10 m-[1px] rounded-lg inline-flex h-11 transform-gpu cursor-pointer items-center justify-center overflow-hidden px-6 py-2 text-sm leading-none font-semibold will-change-transform outline-none",
      colors.button,
      colors.textColor,
      colors.textShadow,
    ),
...
const ShineEffect = ({ isPressed }: { isPressed: boolean }) => {
  return (
    ...
    >
      <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-transparent via-neutral-100 to-transparent" />
    </div>
  );
};
