...
export function ApiBillsManager({ projectId }: ApiBillsManagerProps) {
  ...
  const getProviderBadge = (provider: string) => {
    ...
    if (p.includes('glm')) return <Badge variant="outline" className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-[10px] font-mono">GLM</Badge>;
    if (p.includes('groq')) return <Badge variant="outline" className="bg-orange-500/10 text-orange-400 border-orange-500/20 text-[10px] font-mono">GROQ</Badge>;
    if (p.includes('gemini')) return <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/20 text-[10px] font-mono">GEMINI</Badge>;
    if (p.includes('openai')) return <Badge variant="outline" className="bg-purple-500/10 text-purple-400 border-purple-500/20 text-[10px] font-mono">OPENAI</Badge>;
    ...
    <div className="space-y-6">
      ...
      <Card className="border border-border/70 bg-gradient-to-br from-card/90 via-card/50 to-background shadow-xl backdrop-blur-md overflow-hidden">
        ...
        <CardContent className="p-6 space-y-6">
          ...
          {/* ── 3. Live Request & Billing Ledger Table ── */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  Showing recent API calls with real token counts, execution latency, and exact cost estimates.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="h-8 px-2 text-xs font-mono rounded-md border border-border/60 bg-secondary/30 text-foreground"
                >
